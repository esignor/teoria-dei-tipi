...
\UnaryInfC{? $\in$ $\forall_{z \in Nat}$ Id(Nat,0+z,z)[z $\in$ Nat]}
\end{prooftree}
\end{adjustwidth}
\vspace{0.3cm}
\noindent
...
El$_{Nat}$(succ(m),c,e) $\equiv$ El$_{Nat}$(succ(z),0,(w,u).succ(u))\\
e(m,El$_{Nat}$(m,c,e)) $\in$ M(succ(m))[$\Gamma$] $\equiv$ succ(El$_{Nat}$(z,0,(w,u).succ(u)) $\in$ Nat [$\Delta$] $\equiv$ succ(0+z) $\in$ Nat[$\Delta$]
\small
\begin{adjustwidth}{-12em}{}
\begin{prooftree}
\AxiomC{\textbf{$\bigstar$}}
...
\LeftLabel{C$_2$-Nat}
\QuaternaryInfC{0+succ(z)  $=$ succ(0+z) $\in$ Nat[$\Delta$]}
\LeftLabel{sym}
\UnaryInfC{succ(0+z) $=$ 0+succ(z) $\in$ Nat[$\Delta$]}
