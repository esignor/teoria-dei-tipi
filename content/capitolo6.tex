...
\textbf{Soluzione}\\\\
\noindent
pf $\in$ $\forall_{z \in A}$ $\forall_{w_2 \in Id(A,y,z)}$ Id(A,x,z)[x $\in$ A,y $\in$ A,w$_1$ $\in$ Id(A,x,y)] $\equiv$ pf $\in$ {\scriptsize$\prod$}$_{z \in A}$ {\scriptsize$\prod$}$_{w_2 \in Id(A,y,z)}$ Id(A,x,z)[x $\in$ A,y $\in$ A,w$_1$ $\in$ Id(A,x,y)]\\\\
Uso l'eliminatore dell'identit\`a dipendente (\textit{E-Id$_{dip}$}\\
\begin{itemize}
\item Premesse:
\begin{itemize}
\item M(z$_1$,z$_2$,z$_3$)type[$\Gamma,z_1 \in A,z_2 \in A,z_3 \in Id(A,z_1,z_2,z_3)]$ $\equiv$ {\scriptsize$\prod$}$_{z \in A}$ {\scriptsize$\prod$}$_{w_2 \in Id(A,y,z)}$ Id(A,x,z)[x $\in$ A,y $\in$ A,w$_1$ $\in$ Id(A,x,y)]
\item e(x) $\in$ M(x,x,id(x))[$\Gamma,x \in A$] $\equiv$
\end{itemize}
\item Conclusione:
\begin{itemize}
\item El$_{Id}$(z$_3$, (x).e(x)) $\equiv$ El$_{Id}$(w$_1$, (x).e(x))
