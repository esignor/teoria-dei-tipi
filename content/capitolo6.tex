...
\textit{Nel dettaglio l'esercizio presentato in questa sezione, e svolto a lezione, viene risolto in \S{sec: es-id}, esercizio 5}
...
\UnaryInfC{? $\forall_{z \in Nat}$ Id(Nat,0+z,z)[ ]}
\end{prooftree}
\end{adjustwidth}
\noindent
\vspace{0.3cm}
...
\item Conclusioni:
\begin{itemize}
\item El$_{Nat}$(z,c,e) $\equiv$ El$_{Nat}$(x,id(0),(z,v).h(z+0,z,v))
...
\UnaryInfC{id(0) $\in$ Id(Nat,0+0,0)[ ]}
\AxiomC{\textbf{3}}
\UnaryInfC{h(z+0,z,v) $\in$ Id(Nat,succ(z)+0,succ(z))[z $\in$ Nat,v $\in$ Id(Nat,z+0,z)]}
\LeftLabel{E-Nat$_{dip}$}
\TrinaryInfC{El$_{Nat}$(x,id(0),(z,v).h(z+0,z,v))Id(Nat,x+0,x)[x $\in$ Nat]}
\end{prooftree}
\end{adjustwidth}
\vspace{0.5cm}
\normalsize \textbf{1}
...
\vspace{0.5cm}
\normalsize \textbf{3}\\\\
$\Delta$ $\equiv$ z $\in$ Nat,v $\in$ Id(Nat,z+0,z)
\small
\begin{adjustwidth}{-12em}{}
\begin{prooftree}
\AxiomC{\textbf{$\diamondsuit$}}
\UnaryInfC{h(z+0,z,v) $\in$ Id(Nat,succ(z+0),succ(z))[$\Delta$]}
\AxiomC{\textbf{3$^\backprime$}}
\UnaryInfC{Id(Nat,succ(z+0),succ(z)) $=$ Id(Nat,succ(z)+0,succ(z))[$\Delta$]}
\LeftLabel{conv}
\BinaryInfC{h(z+0,z,v) $\in$ Id(Nat,succ(z)+0,succ(z))[$\Delta$]}
\end{prooftree}
\end{adjustwidth}
...
\UnaryInfC{Id(Nat,z+0,z)type[z $\in$ Nat]}
\LeftLabel{F-c}\RightLabel{\begin{tabular}[c]{cc}(y $\in$ Id(Nat,z+0,z))\\ $\notin$ z $\in$ Nat\end{tabular}}
...
\UnaryInfC{succ(z+0) $=$ succ(z)+0 $\in$ Nat[$\Delta$]}
...
\LeftLabel{eq-F-Id}
\TrinaryInfC{Id(Nat,succ(z+0),succ(z)) $=$ Id(Nat,succ(z)+0,succ(z))[$\Delta$]}
\end{prooftree}
\end{adjustwidth}
...
\LeftLabel{F-Id}
\TrinaryInfC{Id(Nat,z+0,z)type[z $\in$ Nat]}
...
\AxiomC{\textbf{$\heartsuit$}}
\UnaryInfC{z+0 $\in$ Nat[z $\in$ Nat]}
\AxiomC{\textbf{$\bigstar$}}
\UnaryInfC{$\Delta$ cont}
\LeftLabel{ind-te}
\BinaryInfC{z+0 $\in$ Nat[$\Delta$]}
\LeftLabel{I$_2$-Nat}
\UnaryInfC{succ(z+0) $\in$ Nat[$\Delta$]}
...
\LeftLabel{C$_1$-Nat}
\TrinaryInfC{succ(z)+0  $=$ succ(z+0) $\in$ Nat[$\Delta$]}
\LeftLabel{sym}
\UnaryInfC{succ(z+0) $=$ succ(z)+0 $\in$ Nat[$\Delta$]}
