...
\begin{prooftree}
\AxiomC{M(x) type[$\Gamma$, x $\in$ A]}
\UnaryInfC{M(w) type[$\Gamma$, w $\in$ A]}
\end{prooftree}
\noindent
\\\\
\textbf{Soluzione}\\\\
\noindent
Assumo per ipotesi che A sia un tipo dipendente (A $\equiv$ Nat), vale per\`o per qualsiasi teoria dei tipi.
...
\AxiomC{\textbf{1}}
\UnaryInfC{M(x) type[$\Gamma$, x $\in$ Nat, w $\in$ Nat]}

\AxiomC{\textbf{2}}
\UnaryInfC{$\Gamma$, w $\in$ Nat, x $\in$ Nat cont}

\LeftLabel{ex-ty}
\BinaryInfC{M(x) type[$\Gamma$, w $\in$ Nat, x $\in$ Nat]}

\LeftLabel{sub-typ}
\BinaryInfC{M(w) type[$\Gamma$, w $\in$ Nat]}
\end{prooftree}
\noindent
\normalsize \textbf{1}
\small
...
\UnaryInfC{Nat type[$\Gamma$, x $\in$ Nat]}
\LeftLabel{F-c}\RightLabel{(w $\in$ Nat) $\notin$ x $\in$ Nat}
\UnaryInfC{$\Gamma$, x $\in$ Nat, w $\in$ Nat cont}
\LeftLabel{ind-ty} 
\BinaryInfC{M(x) type[$\Gamma$, x $\in$ Nat, w $\in$ Nat]}
\end{prooftree}
\noindent
\normalsize \textbf{2}
\small
...
\UnaryInfC{$\Gamma$, w $\in$ Nat cont}
\LeftLabel{F-Nat}
\UnaryInfC{Nat type[$\Gamma$, w $\in$ Nat]}
\LeftLabel{F-c}\RightLabel{(x $\in$ Nat) $\notin$ w $\in$ Nat}
\UnaryInfC{$\Gamma$, w $\in$ Nat, x $\in$ Nat cont}
\end{prooftree}
\noindent
\normalsize
$\Gamma$ cont \`e assioma ([ ] cont $\equiv$ $\Gamma$ = $\varnothing$).