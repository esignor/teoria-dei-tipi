...
\AxiomC{e(x,w,y) $\in$ M(cons(x,w))[$\Gamma$, x $\in$ List(A),  w $\in$ A, y $\in$ M(x)]}
\LeftLabel{C$_1$-list)}
\TrinaryInfC{El$_{list}$(nil,c,e) = c $\in$ M(nil)[$\Gamma$]}
\end{prooftree}
\end{adjustwidth}
\scriptsize
...
\AxiomC{e(x,w,y) $\in$ M(cons(x,w))[$\Gamma$, x $\in$ List(A), w $\in$ A, y $\in$ M(x)]}
\LeftLabel{C$_2$-list)}
\QuinaryInfC{El$_{list}$(cons(s,a),c,e) = e(s,a, E$_list$(s,c,e) $\in$ M(cons(s,a))[$\Gamma$]}
\end{prooftree}
\end{adjustwidth}
...
\end{adjustwidth}
\scriptsize
\begin{adjustwidth}{-18.5em}{}
\begin{prooftree}
\AxiomC{M(z) type[$\Gamma$, z $\in$ List(A)]}
\AxiomC{t$_1$ = t$_2$ $\in$ list(A)[$\Gamma$]}
\AxiomC{c$_1$ = c$_2$ $\in$ M(nil)[$\Gamma$]}
\AxiomC{e$_1$(x,w,y) = e$_2$(x,w,y) $\in$ M(cons(x,w))[$\Gamma$, x $\in$ List(A), w $\in$ A, y $\in$ M(x)]}
\LeftLabel{E-eq-List)}
\QuaternaryInfC{El$_{list}$(t$_1$, c$_1$, e$_1$) = El$_{list}$(t$_2$, c$_2$, e$_2$ $\in$ M(t$_1$)[$\Gamma$]}
\end{prooftree}
\end{adjustwidth}
