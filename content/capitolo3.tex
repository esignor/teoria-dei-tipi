...
\AxiomC{e(x,w,y) $\in$ M(cons(x,w))[$\Gamma$, x $\in$ List(A), w $\in$ A, y $\in$ M(x)]}
\LeftLabel{C$_2$-list)}
\TrinaryInfC{El$_{list}$(cons(s,a),c,e) = e(s,a, E$_list$(s,c,e) $\in$ M(cons(s,a))[$\Gamma$]}
\end{prooftree}
\end{adjustwidth}
...
\noindent tale che y$\backprime$ venga posto all'inizio di della lista x$\backprime$ allora come occorre procedere?.}\\\\
...
in modo tale che valga append(x,null) = x $\in$ List(A)[x $\in$ List(A)]
