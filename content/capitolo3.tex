...
Le regole di formazione dei tipi e dei loro termini sono formulate, in modo, da rendere la regole di sostituzione per tipi e termini ammissibili.\\
...
\begin{center}
\textbf{
rec(n$_1$...n$_m$, 0) ${\overset{\mathit{def}}{=}}$ g$_0$(n$_1$...n$_m$)\\
rec(n$_1$...n$_m$, k+1) ${\overset{\mathit{def}}{=}}$ g$_0$(n$_1$...n$_m$, k, rec(n$_1$...n$_m$, k))
}
\end{center}
...
\LeftLabel{I$_2$-Nat$_{prog}$}
\UnaryInfC{succ(y) $\in$ Nat [w $\in$ Nat, x $\in$ Nat, y $\in$ Nat]}
\LeftLabel{E-Nat$_{dip}$}
...
El$_{Nat}$(succ(0), w, (x,y).succ(y)) $\rightarrow_1$ succ(El$_{Nat}$(0, w, (x,y).succ(y))) per \textit{$\beta_{1Nat}$-red} $\rightarrow_1$ succ(w)\\
...
\noindent
\begin{itemize}
\item In I$_2$-Nat$_{prog}$ il contesto \`e $\Sigma$ $\equiv$ $\Gamma$, x $\in$ Nat che permette l'applicazione di I$_2$ Nat.
\begin{prooftree}
\AxiomC{$\Gamma$ cont}
\LeftLabel{F-Nat}
\UnaryInfC{Nat type[$\Gamma$]}
...
\vspace{0.5cm}
\noindent
\normalsize{Assumo che le premesse di \textit{I$_2$-Nat$_{prog}$} (\textit{$\Gamma$ count}) siano valide, perci\`o \`e valido, dalla prova sopra, anche il giudizio di conclusione \textit{succ(x) $\in$ Nat[$\Gamma$, x $\in$ Nat]}, di conseguenza derivabile in \textit{T$^\backprime$}.}
\item In E-Nat$_{dip}$ il contesto \`e $\Sigma$ $\equiv$ $\Gamma$, z $\in$ Nat, che permette l'applicazione di E-Nat.
\scriptsize
...
\noindent
\normalsize
Assumo che le premesse di \textit{E-Nat$_{dip}$} (\textit{M(z) type[$\Gamma$, z $\in$ Nat], c $\in$ M(0)[$\Gamma$], e(x,y) $\in$ M(succ(x))[$\Sigma$, x $\in$ Nat, y $\in$ M(x)]}) siano valide e che $\Gamma$ cont sia assioma ([ ] cont $=$ [$\Gamma$] cont con $\Gamma$ $=$ $\varnothing$), perci\`o \`e valido, dalla prova sopra, anche il giudizio di conclusione \textit{El$_{Nat}$(z,c,e) $\in$ M(z)[$\Gamma$, z $\in$ Nat]}, di conseguenza derivabile in \textit{T$^\backprime$}.
\end{itemize}
\pagebreak
...
\\\\
\textbf{Soluzione}\\\\
La ricorsione la faccio su w, usando lo schema di ricorsione primitiva, vale che w + 2 $\equiv$ El$_{Nat}$(w, 2, (x,y).succ(y)) $\in$ Nat[w $\in$ Nat].
...
\LeftLabel{I$_2$-Nat$_{prog}$}
\UnaryInfC{succ(z) $\in$ Nat[y $\in$ Nat, w $\in$ Nat, z $\in$ Nat]}
\LeftLabel{E-Nat$_{dip}$}
...
\textbf{Soluzione}\\\\
\noindent
Assumo, per ipotesi, che A sia un tipo non dipendente (A $\equiv$ Nat). Vale per\`o per qualsiasi tipo.
