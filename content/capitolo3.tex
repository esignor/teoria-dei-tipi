...
\LeftLabel{ind-te}
\BinaryInfC{e(x,y) $\in$ M(succ(x))[$\Gamma$, x $\in$ Nat, y $\in$ M(x), z $\in$ Nat]}

\AxiomC{}
\UnaryInfC{M(x) type[$\Gamma$, z $\in$ Nat]}
\AxiomC{\textbf{2$_B$}}
...
\UnaryInfC{$\Gamma$, z $\in$ Nat, x $\in$ Nat, y $\in$ M(x) cont}
\LeftLabel{ex-te}
\BinaryInfC{e(x,y) $\in$ M(succ(x))[$\Gamma$, z $\in$ Nat, x $\in$ Nat, y $\in$ M(x)]}
\end{prooftree}
\end{adjustwidth}
\noindent
\textbf{2$_A$}
...
\LeftLabel{sub-typ}
\BinaryInfC{M(x) type[$\Gamma$, x $\in$ Nat]}
...
\vspace{0.5cm}
\noindent
\normalize Le derivazioni contrassegnate con ($\ast$) indicano che le ho gi\`a svolte all'interno dell'esercizio e che le ho omesse per evitare ripetizioni.\\
