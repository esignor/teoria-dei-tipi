...
Devo verificare che pf $\in$ $\prod_{z \in \varphi(\ast)}$ $\forall_{x \in N_1}$ $\varphi$(x)[$\Gamma$] $\equiv$ pf $\in$ $\prod_{z \in \varphi(\ast)}$ $\prod_{x \in N_1}$ $\varphi$(x)[$\Gamma$] che equivale a \\
...
\LeftLabel{E-{\scriptsize $\prod$}}
\BinaryInfC{(Ap(Ap(Ap(k,x),w),y)) $\in$ $\varphi(cons(x,w))$[$\Delta$, x $\in$ List(A), w $\in$ A, y $
\in$ $\varphi(x)$]}
\end{prooftree}
\end{adjustwidth}
...
\LeftLabel{E-$+$}
\TrinaryInfC{El$_{B+C}$(z,Ap(k,x$_1$),Ap(w,x$_2$)) $\in$ $\varphi(z)$ $[\Delta, z \in B + C]$}
\end{prooftree}
\end{adjustwidth}
\noindent
\normalsize
...
Devo verificare che pf $\in$ {\scriptsize $\prod$}$_{x \in B}$ {\scriptsize $\prod$}$_{y \in C(x)}$ $\varphi(<x,y>)$ $\rightarrow$ {\scriptsize $\prod_{z \in \sum_{x \in B} C(x)}$} $\varphi(z)$ [$\Gamma$] $\equiv$ pf $\in$ {\small $\prod_{z^\backprime \in ({\small \prod}_{x \in B} {\small \prod}_{y \in C(x)} \varphi(<x,y>))}$} {\small $\prod_{z \in \sum_{x \in B} C(x)}$} $\varphi(z)$ [$\Gamma$] che equivale a \\
$\lambda z.El_\Sigma(z,Ap(Ap(z^\backprime,x),y))$ $\in$ {\scriptsize $\prod_{z \in \sum_{x \in B} C(x)}$} $\varphi(z)$[$\Gamma$, z $^\backprime \in ({\scriptsize \prod}_{x \in B} {\scriptsize \prod}_{y \in C(x)} \varphi(<x,y>))$\\
Applico \textit{(I-\scriptsize $\prod$)} e ottengo \\
