...
\begin{center}
($\bot$)$^I$[$\Gamma$] ${\overset{\mathit{def}}{\equiv}}$ N$_0$ type[$\Gamma$]\\
(\textit{tt})$^I$[$\Gamma$] ${\overset{\mathit{def}}{\equiv}}$ N$_1$ type[$\Gamma$]\\
...
($\neg$ $\varphi$)$^I$[$\Gamma$] ${\overset{\mathit{def}}{\equiv}}$ $\varphi^I$ $\rightarrow$ $\bot^I$type[$\Gamma$] ${\overset{\mathit{def}}{\equiv}}$ $\varphi^I$ $\rightarrow$ N$_0$ type[$\Gamma$]\\
($\forall_{x \in A}$ $\varphi$(x))[$\Gamma$] ${\overset{\mathit{def}}{\equiv}}$ $\prod\limits_{x \in A}$($\varphi$(x))$^I$ type[$\Gamma$]\\
(t $=$ s)$^I$[$\Gamma$] ${\overset{\mathit{def}}{\equiv}}$ Id(A,t,s) type[$\Gamma$] $\Rightarrow$ \textit{unico predicato atomico}\\
($\exists_{x \in A}$ $\varphi$(x))$^I$[$\Gamma$] ${\overset{\mathit{def}}{\equiv}}$ $\sum\limits_{x \in A}$($\varphi$(x))$^I$ type[$\Gamma$]
\end{center}
\noindent
...
\begin{center}
\textbf{$\alpha$ prop} (secondo il solito uso di proposizione) \quad \textbf{sse} \quad \textbf{$\alpha$ prop[$\Gamma$]}
\end{center}
\vspace{0.3cm}
\textit{Definizione}\\\\
\noindent
Introduciamo il concetto di \textbf{dimostrazioni logiche}.\\
Date le proposizioni $\phi$ prop[$\Gamma$] e $\alpha_i$ prop[$\Gamma$] per i $=$ 1,..,n derivabili in teoria dei tipi.\\
...
\noindent
Introdotto il giudizio diciamo che \`e derivabile se esiste un termine (detto \textit{proof-term}) pf tale che pf $\in$ $\phi$[$\Gamma$,x$_1$ $\in$ $\alpha_1$,...,x$_n$ $\in$ $\alpha_n$] che \`e derivabile in una teoria dei tipi.\\\\
...
\\\\
\noindent
\textbf{Teorema} i sequenti derivabili, del calcolo della deduzione naturale \textit{DNI},\footnote{Logica Intuizionistica.} sono validi in teoria dei tipi.\\\\
\noindent
...
\begin{center}$\forall_{x \in A}$ $\exists_{y \in B} R(x,y)$ $\rightarrow$ $\exists_{f \in A \rightarrow B}$ $\forall_{x \in A}$ $R(x,f(x)$) true[$\Gamma$]\end{center}
}
\noindent
\textbf{Soluzione}
\\\\
...
In logica predicativa con l'uguaglianza diventa:\\\\
\noindent
...
\item terza applicazione, considerando $\prod_{z \in (\prod_{x \in A} \hspace{0.2cm} \sum_{y \in B} R(x,y))}$ $\sum_{f \in A \rightarrow B}$ C(f) [$\Gamma$] devo individuare un elemento che appartenga a B: $\lambda$z.$<\pi_1$($Ap(z,x)$), $\pi_2$($Ap(z,x)$)$>$.c(f)
...
\UnaryInfC{A type[$\Gamma$]}
\AxiomC{}
\UnaryInfC{$\sum_{y \in B} R(x,y)$ type[$\Gamma$,x $\in$ A]}
\LeftLabel{F-{\scriptsize$\prod$}}
\BinaryInfC{$\prod_{x \in A}$ $\sum_{y \in B} R(x,y)$ type[$\Gamma$]}
\LeftLabel{F-c}\RightLabel{(z $\in$ $\prod_{x \in A}$ $\sum_{y \in B} R(x,y)$) $\notin$ $\Gamma$}
\UnaryInfC{$\theta$ cont}
\LeftLabel{ind-ty}
