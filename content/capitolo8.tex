...
\begin{center}$\forall_{x \in B}$ $\varphi$(x) ${\overset{\mathit{def}}{=}}$ $\prod_{x \in B}$ $\varphi$(x)\end{center}
Questo risulta vero dalle regole \textit{(I-{\scriptsize$\prod$})} e \textit{(El-{\scriptsize$\prod$})}.\\
\begin{prooftree}
\AxiomC{c(x) $\in$ $\varphi$(x)[$\Gamma$,x $\in$ B]}
\LeftLabel{I-{\scriptsize$\prod$})}
\UnaryInfC{? $\in$ $\prod_{x \in B}$ $\varphi$(x)[$\Gamma$]}
\end{prooftree}
\noindent
definisce nella \textit{set-theory} la regola del $\forall$. Difatti la regola sopra, con le opportune sostituzioni diventa
\begin{prooftree}
...
\end{prooftree}
\noindent
con $\Delta$ $\equiv$[x$_1$...$\varphi_1$...$x_n$...$\varphi_n$]\\
Nel calcolo dei sequenti equivale a
\begin{prooftree}
\AxiomC{$\Delta$ $\vdash_\Gamma,x \in B$ $\varphi(x)$}
\LeftLabel{$\forall$-D)}\RightLabel{x $\in$ vL($\Delta$)}
\UnaryInfC{$\Delta$ $\vdash_\Gamma$ $_{x \in B}$ $\varphi(x)$}
\end{prooftree} 

\begin{prooftree}
\AxiomC{f $\in$ $\prod_{x \in B}$ $\varphi$(x)[$\Gamma$]}
\AxiomC{b $\in$ B[$\Gamma$]}
\LeftLabel{E-{\scriptsize$\prod$})}
\BinaryInfC{? $\in$ $\varphi$(b)[$\Gamma$]}
\end{prooftree}
\noindent
con le opportune sostituzioni diventa, nel calcolo dei sequenti, una regola di eliminazione della riduzione Naturale
\begin{prooftree}
...
\noindent
\\\\
In conclusione, il prodotto dipendente definisce sia l'implicazione
\begin{center} $\beta$ $\rightarrow$ $\psi$ $\equiv$ $\forall_{x \in B}$ $\varphi$ \end{center}
dove $\rightarrow$ indica famiglia costante\\
che la quantificazione esistenziale
\begin{center}$\forall_{x \in B}$ $\varphi(x)$ $\equiv$ $\prod_{x \in B}$ $\varphi(x)$ \end{center}
