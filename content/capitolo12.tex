...
\noindent
Suppongo W \textit{set}, allora si dimostra come \textit{lemma cruciale} che W \`e ben fondato. In quanto se esistesse una cantena (y$_i$)$_{i \in Nat}$ w$\in$y$_0\in$y$_i$... $\in$y$_i\in$y$_{i+1}$ per il fatto che y$_i$ \`e un elemento di w allora \`e ben fondato. Ma riusciremmo ad avere che la catena (y$_i$)$_i>=1$ \`e infinita e questo porterebbe a una contraddizione.\\\\
\noindent
...
\noindent
\textit{Conclusione}: non esiste W $=$ \{X \textit{set} $|$ X \`e ben fondato\}. Ecco che in \textbf{MLTT} $+$ $\hat{U}_0$ $in$ U$_0$ \`e inconsistente, ovvero $\bot$ true [ ] in tale teoria.
\\\\
\noindent
