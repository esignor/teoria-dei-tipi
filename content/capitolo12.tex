...
Nella prima versione \textit{Martin-L$\ddot{o}$f} permetteva che U$_0$ $\in$ U$_0$, generando per\`o una contraddizione.
\textbf{\begin{center} U$_0$ $\notin$ U$_0$ \quad $\rightarrow$ \quad U$_0$ $\in$ U$_0$ \`e una contraddizione\end{center}}
Universo, nel senso di  \textit{Martin-L$\ddot{o}$f},  \`e un contenitore, di tipi, predicativo. Predicativo, ovvero U$_0$ $\notin$ U$_0$.\\\\
\noindent
\textit{Fondamentale}\\
...
\noindent
Il tipo del \textbf{Primo Universo (U$_0$) alla Tarski} \`e definito dalle regole seguenti, dove \textbf{T} indica la decodifica del codice a, e trasforma il codice in un tipo.
...
\end{center}
\noindent
\textit{Dove b rappresenta la codifica di un tipo in un contenitore e $\hat{List}(b)$ il codice delle liste associate a b.}
\vspace{0.5cm}
\begin{center}
\begin{prooftree}
\AxiomC{b $\in$ U$_0$[$\Gamma$]}
...
\end{prooftree}
\end{center}
\textit{Per le regole di Introduzione va sottolineato che U$_0$ funge da contenitore di tipi, tramite codifica, ed \`e chiuso rispetto ai tipi finora introdotti, nei capitolo precedenti. Inoltre le regole di Introduzione usano quelle di Eliminazione.}
...
\begin{itemize}
\item $\beta_{Uno}$-I$_1$-red) \textbf{T}($\hat{N_0}$) $\rightarrow_1$ N$_0$
\item $\beta_{Uno}$-I$_2$-red) \textbf{T}($\hat{N_1}$) $\rightarrow_1$ N$_1$
\item $\beta_{Uno}$-I$_3$-red) \textbf{T}($\hat{Nat}$) $\rightarrow_1$ Nat
\item $\beta_{Uno}$-I$_4$-red) \textbf{T}($\hat{\sum}_{x \in b} c(x)$) $\rightarrow_1$ $\sum_{x \in \textbf{T}(b)} \textbf{T}(c(x))$
\item $\beta_{Uno}$-I$_5$-red) \textbf{T}($\hat{\prod}_{x \in b} c(x)$) $\rightarrow_1$ $\prod_{x \in \textbf{T}(b)} \textbf{T}(c(x))$
\item $\beta_{Uno}$-I$_6$-red) \textbf{T}(b $\hat{+}$ c) $\rightarrow_1$ \textbf{T}(b) $+$ \textbf{T}(c)
\item $\beta_{Uno}$-I$_7$-red) \textbf{T}($\hat{List}$(b)) $\rightarrow_1$ List(\textbf{T}(b))
\item $\beta_{Uno}$-I$_9$-red) \textbf{T}($\hat{Id}(b,c,d)$) $\rightarrow_1$ Id(\textbf{T}(b),c,d)
