...
\section{Principi di scelta per estrazioni di programmi da dimostrazioni}
...
\begin{center}\textbf{$\Omega$ $\equiv$ $W$ ${\overset{\mathit{def}}{=}}$  \{(X,c) tipi $|$ (X,c) \`e ben fondato\}} \end{center}
...
\textbf{Soluzione}\\\\
\noindent
\textit{Devo dimostrare che esiste pf $\in$ Id(Nat,0,1) true[ ] $\equiv$ pf(z) $\in$ N$_0$[z $\in$ Id(Nat,0,1)], come gi\`a dimostrato in \S\ref{sec:esercizio-su-universo}. Per completezza, di seguito, mostro la derivabilit\`a di $\neg$ Id(Nat,0,1)type[ ].}\\\\
\noindent
