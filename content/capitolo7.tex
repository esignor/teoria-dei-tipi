...
%17 inclusa
\normalsize
La somma indiciata forte \`e il potenziamento indiciato della \textbf{somma disgiunta binaria} (\S\ref{cap: tipo-somma-disgiunta}). Tipo induttivo, ovvero generato con il principio d'induzione della regola di eliminazione, di tipi dipendente.\\\\
\noindent
\textit{Definizione set-teorica}\\
\noindent
\textbf{
\begin{center}$\bigcup\limits_{x \in B\hspace{0.1cm}set}^.$C(x) \qquad (C(x) set) x $\in$ B \end{center}
...
\LeftLabel{I-$\Sigma$)}
\TrinaryInfC{$<$b,c$>$ $\in$ $\sum\limits_{x\in B}$ C(x)[$\Gamma$]}
\end{prooftree}
...
\TrinaryInfC{$<$b$_1$,c$_1$$>$ $=$ $<$b$_2$,c$_2$$>$ $\in$ $\sum\limits_{x \in B}$ C(x)[$\Gamma$]}
\end{prooftree}
...
\LeftLabel{PJ$_1$-$\times$)}
\UnaryInfC{$\pi_1$d $\in$ A[$\Gamma$]}
\end{prooftree}
\begin{prooftree}
\AxiomC{d $\in$ A $\times$ B[$\Gamma$]}
\LeftLabel{PJ$_2$-$\times$)}
...
\LeftLabel{F-$\Sigma$}
\BinaryInfC{B $\times$ C ${\overset{\mathit{def}}{=}}$ $\sum\in_{x \in B}$C}
\end{prooftree}
\noindent
...
\item $\beta_\times$-red$_3$) $\pi_2$($<$b,c$>$) $\rightarrow_1$ c
\item \AxiomC{t$_1$ $\rightarrow_1$ t$_2$}
\LeftLabel{$\times$-red)}
\UnaryInfC{El$_\Sigma$ (t$_1$, e) $\rightarrow_1$ El$_\Sigma$ (t$_2$, e)}
\DisplayProof \qquad
...
\textit{Dimostrazione}\\\\
\noindent
Dimostro che B $\times$ C ${\overset{\mathit{def}}{=}}$ $\sum\limits_{x \in B\hspace{0.1cm}set}$C, trovando \textit{(PJ$_1$-$\times$)} e \textit{(PJ$_2$-$\times$)} usando \textit{(E-$\Sigma_{dip}$)}.

\begin{center}
M(z) $\equiv$ B\\
\noindent
$\sum\limits_{x \in B}$C $\equiv$ B $\times$ C
\end{center}
\noindent
...
\LeftLabel{F-$\times$}
\BinaryInfC{$\sum\limits_{x \in B}$C type[$\Gamma$]}
\LeftLabel{F-c}\RightLabel{(z $\in$ $\sum\limits_{x \in B}$C) $\notin$ $\Gamma$}
\UnaryInfC{$\Gamma$,z $\in$ $\sum\limits_{x \in B}$C cont}
\LeftLabel{ind-ty}
\BinaryInfC{B type[$\Gamma$,z $\in$ $\sum\limits_{x \in B}$C]}
...
\LeftLabel{E-$\Sigma_{dip}$}
\BinaryInfC{$\pi_1$z $\in$ B[$\Gamma$,z $\in$ $\sum\limits_{x \in B}$C]}
\end{prooftree}
\end{adjustwidth}
\noindent
\normalsize
...
\noindent
In conclusione
\begin{center}$\pi_1$z ${\overset{\mathit{def}}{=}}$ El$_\Sigma$(z,(x,y).x) $\in$ B[$\Gamma$,z $\in$ $\sum\limits_{x \in B}$C]\end{center}
...
\noindent
La seconda proiezione sulla coppia canonica M($<$x,y$>$) \`e y\\\\
\small
...
\AxiomC{\textbf{($\ast$)}}
\UnaryInfC{B type[$\Gamma$, z $\in$ $\sum\limits_{x \in B}$C]}
...
\LeftLabel{E-$\Sigma_{dip}$}
\BinaryInfC{$\pi_2$z $\in$ C[$\Gamma$,z $\in$ $\sum\limits_{x \in B}$C]}
\end{prooftree}
\end{adjustwidth}
\noindent
\normalsize
...
\noindent
In conclusione
\begin{center}$\pi_2$z ${\overset{\mathit{def}}{=}}$ El$_\Sigma$(z,(x,y).y) $\in$ C[$\Gamma$,z $\in$ $\sum\limits_{x \in B}$C]\end{center}
\noindent
Ho concluso con \textbf{($\ast$)} i sequenti gi\`a risolti, per evitare ripetizioni.\\
\end{enumerate}
Per verificare che effettivamente \textbf{$\pi_1$z} e \textbf{$\pi_2$z} siano \textit{(PJ$_1$-$\times$)} e \textit{(PJ$_2$-$\times$)}, utilizzo le riduzioni \textit{$\beta_\times$-red$_1$} e \textit{$\beta_\times$-red$_2$}.\\
...
\end{itemize}
\noindent
Entrambe le \textit{$\beta_\times$-red} sono verificate. Percui la definizione B $\times$ C ${\overset{\mathit{def}}{=}}$ $\sum\limits_{x \in B}$C \`e giustificata.

\subsection{Esempi}
\label{subsec:esempi}
L'unione disgiunta non \`e l'unione insiemistica e questo lo si vede chiaramente dalla relazione sotto\\
\noindent
$\bigcup\limits_{x \in Nat}^.$Nat $\equiv$ \{(x,y) $\bullet$ x $\in$ Nat,y $\in$ Nat\} $\equiv$ Nat $\times$ Nat $\neq$ $\bigcup\limits_{x \in Nat}$Nat \{y: y $\in$ Nat, per un certo x $\in$ Nat\}\\
\noindent
La prima parte della relazione, se la famiglia \`e costante ingloba il prodotto cartesiano.\\\\
\noindent
Un esempio di unioni genuine, che si possono formare con tipi che non coincidono con il prodotto cartesiano, \`e $\sum\limits_{x \in Nat}$ Mat(x $\bullet$ x), ove Mat sono le matrici quadratiche.\\
A livello insiemisitico potremmo dover proiettare, data una coppia $<$x,k$>$ (con x elementi e k una certa matrice), sull'indice o sull'elemento di cui stiamo parlando. Dunque dovremmo poter avere a disposizione le due proiezioni \textit{(PJ$_1$-$\times$)} e \textit{(PJ$_2$-$\times$)} anche su famiglie non costanti.\\\\
\noindent
Per individuare correttamente $\pi_1$(z) e $\pi_2$(z) assumo che $\sum\limits_{x \in B}$C(x) type[$\Gamma$] sia derivabile. Tale ipotesi \`e vincolante per l'ottenimento della famiglia. \\\\
\noindent
Definisco $\omega$ $=$ z $\in$ $\sum\limits_{x \in B}$C(x)
\begin{enumerate}
...
\normalsize
In conclusione
\begin{center}$\pi_1$z ${\overset{\mathit{def}}{=}}$ El$_\Sigma$(z,(x,y).x) $\in$ B[$\Gamma$,$\omega$]\end{center}

\item Per la seconda proiezione\\
\noindent
$\pi_2$z ${\overset{\mathit{def}}{=}}$ El$_\Sigma$(z,(x,y).y) $\in$ C($\pi_1(z)$)[$\Gamma$,$\omega$]\\\\
\noindent
...
\textbf{\textbf{1}}
\small
\begin{adjustwidth}{-16.5em}{}
\begin{prooftree}
\AxiomC{\textbf{($\ast$)}}
\UnaryInfC{z $\in$ $\sum\limits_{x \in B}$C(x)[$\Gamma$,$\omega$]}
\LeftLabel{PJ$_1$-$\times$}
...
\BinaryInfC{C(x) $=$ C($\pi_1<$x,y$>$) type[$\Gamma$,x $\in$ B,y $\in$ C(x)]}
\LeftLabel{conv}
...
\noindent
Ho concluso con \textbf{($\ast$)} una derivazione per semplificare o evitare ripetizioni nella derivazione stessa.
...
Il tipo \textbf{$\sum\limits_{x \in B}$C(x)} viene usato per rappresentare:
\begin{enumerate}
\item \textbf{uso \textit{set} teoretico:} l'unione indiciata disgiunta insiemistica (C(x) set[x $\in$ B]);
\item \textbf{uso \textit{set} teoretico:} con assioma di separazione;
\item \textbf{uso logico:} proposizionale.
\end{enumerate}
\noindent
...
Perci\`o per ogni y insieme, y \& \{x $\in$ B $|$ $\varphi$(x)\} $\Leftrightarrow$ $\varphi$(y) vale (come gi\`a enunciato in $\S$\ref{sec:paradosso-di-russell}).\\
...
\LeftLabel{F-$\Sigma$}
\UnaryInfC{$\sum\limits_{x \in B}$ $\varphi$(x) $\cancel{type}_{\textbf{set}}$[$\Gamma$]}
\end{prooftree}
\noindent
...
\textit{Dimostrazione}
\begin{itemize}
\item ($\Leftarrow$) da $\sum\limits_{x \in B}$ $\varphi$(x) significa avere b $\in$ B $+$ pf $\in$ $\varphi$(b), e quindi b soddisfa $\varphi$(x). Ecco che non \`e solo b ma $<$b,pf$>$ $\in$ \{x $\in$ B $|$ $\varphi$(x)\} con pf $\in$ $\varphi$(b) $_{I-\Sigma}\Leftarrow$ $\varphi$(b).\\
\item ($\Rightarrow$) z $\in$ \{x $\in$ B $|$ $\varphi$(x)\} con z che sarebbe una coppia e \{x $\in$ B $|$ $\varphi$(x)\} $\equiv$ $\sum\limits_{x \in B}$ $\varphi$(x) $\nRightarrow$ $\varphi$(z) vale, in quanto z \`e una coppia tipata.\\ 
...
\LeftLabel{F-$\Sigma$}
\UnaryInfC{$\sum\limits_{x \in B}$ $\varphi$(x) $\cancel{type}_{\textbf{prop}}$[$\Gamma$] ${\overset{\mathit{def}}{=}} \exists_{x \in B} \varphi(x)$}
\end{prooftree}
\noindent
\`E la \textbf{quantificazione esistenziale}, in logica, di $\varphi$(x) che varia sull'insieme B.\\
\noindent
...
$\equiv \phi$ \`e vero[$\Gamma, \varphi_1$ vero,...,$\varphi_n$ vero] (nessuno degli elementi del contesto dipende dal precedente)
$\equiv$ $\varphi_1,...,\varphi_n$ $\vdash_\Gamma$ $\phi$.
\\\\\\
\noindent
Sappiamo che la logica lavora con variabili \textit{untyped}, dunque noi ora aggiungiamo in  $\exists_{ x\in B}$ $\varphi$(x) l'informazione sul tipo.
\\\\
\noindent
Per quanto assunto sopra, la regola di \textit{Introduzione} viene nel modo sequente riscritta\\\\
\noindent
Assumendo che $\exists_{x \in B}$ $\varphi$(x) type[$\Gamma$] sia ben formato
\begin{prooftree}
\AxiomC{b $\in$ B$_{set}$[$\Gamma$]}
\AxiomC{c $\in$ $\varphi$(b)[$\Gamma$]}
\LeftLabel{I-$\Sigma$}
\BinaryInfC{\cancel{$<b,c>$} $\in$ $\sum\limits_{x \in B}$ C(x)[$\Gamma$]}
\end{prooftree}
\noindent
c $\in$ $\varphi$(b)[$\Gamma$] $\equiv$ $\varphi$(b) \`e vero [$\Gamma$,x$_1$ $\in$ $\varphi_1$,...,x$_n$ $\in$ $\varphi_n$] $\equiv$ $\gamma_1$,...,$\gamma_n$ $\vdash$ $\varphi$(b)\\
$\sum\limits_{x \in B}$ C(x)[$\Gamma$] $\equiv$ $\exists$ x $\in$ B $\varphi$(x) vero[$\Gamma$]\\\\
\noindent
In conclusione, riscritta, la regola di \textit{Introduzione}, con il calcolo dei sequenti diventa\\
\begin{prooftree}
\AxiomC{$\gamma_1$,...,$\gamma_n$ $\vdash_\Gamma$ $\varphi$(b)}
\LeftLabel{I-$\Sigma$}
...
\\\\
\noindent
Ora definisco la regola dell'\textit{Eliminatore dipendente}.\\
\noindent
Per farlo suppongo di avere un'altra proposizione definita come segue
\begin{prooftree}
\AxiomC{$\phi$ prop[$\Gamma$]}
\UnaryInfC{$\phi$ prop[$\Gamma$,z $\in$ $\sum\limits_{x \in B}\varphi(x)$]}
\end{prooftree}
\noindent
...
\noindent
In conclusione, riscritta, la regola dell'\textit{Eliminatore dipendente}, con il calcolo dei sequenti diventa\\
\begin{prooftree}
...
Per
\begin{enumerate}
\item $\sum\limits_{x \in B}C(x)$ \`e un \textit{set} (dove sia x $\in$ B che C(x) sono rispettivamente dei \textit{set}) $\Rightarrow$ B $\times$ C \`e il prodotto cartesiano
\item $\sum\limits_{x \in B}\varphi(x)$ dove $\varphi$ \`e una proposizione $\Rightarrow$ rappresenta l'assioma di separazione \{x $\in$ B $|$ $\varphi$(x)\}\\\
\noindent
Esempio:
$\sum\limits_{z \in List(Nat)}$ Id(Nat,lh(z),2) $\Rightarrow$ rappresenta il sottoinsieme delle liste di lunghezza 2
\item $\exists$ x $\in$ B $\varphi$(x) $\Rightarrow$ $\sum\limits_{x \in B}$ $\varphi$(x), ove $\varphi$(x) e $\sum\limits_{x \in B}$ $\varphi$(x) sono proposizioni
\end{enumerate}
\noindent
Questo serve per:
\begin{itemize}
...
\noindent
Date due proposizioni $\varphi$ prop[$\Gamma$] e $\psi$ prop[$\Gamma$]
\begin{center}
...
$\Rightarrow$ $<a,b>$ $\in$ $\varphi$ $\times$ $\psi$ [$\Gamma$] $\equiv$ $\varphi$ \& $\psi$ vero[$\Gamma$]\\\\
\noindent
...
\noindent
\textbf{$(2)$} Dunque con le \textit{regole di Eliminazione} si \`e mostrato come se $\varphi$ \& $\psi$ true[$\Gamma$] $\Rightarrow$ $\varphi$ true[$\Gamma$] e $\psi$ true[$\Gamma$].
\\\\
\noindent
...
\\\\
\noindent 
In conclusione $\varphi$ \& $\psi$ ${\overset{\mathit{def}}{=}}$  $\varphi$ $\times$ $\psi$ $\equiv$ $\sum\limits_{x \in \varphi}$ $\psi$.\\\\
\noindent
