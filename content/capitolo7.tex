...
\AxiomC{($\ast)$}
\LeftLabel{ind-ty}
\UnaryInfC{B type[$\Gamma$,x $\in$ B,y $\in$ C]}
\AxiomC{($\ast$)}
\UnaryInfC{$\Gamma$,x $\in$ B,y $\in$ C cont}
\LeftLabel{var}
...
A livello insiemisitico potremmo dover proiettare, data una coppia $<$x,k$>$ (con x elementi e k una certa matrice), sull'indice o sull'elemento di cui stiamo parlando, della matrice. Dunque dovremmo poter avere a disposizione le due proiezioni \textit{PJ$_1$-x} e \textit{PJ$_2$-x} anche su famiglie non costanti.