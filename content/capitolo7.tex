...
\UnaryInfC{$\Gamma$,x $\in$ B,y $\in$ C cont}
\LeftLabel{var}
\UnaryInfC{y $\in$ B[$\Gamma$,x $\in$ B,y $\in$ C]}
\LeftLabel{E-$\Sigma_{dip}$}
\BinaryInfC{$\pi_2$z $\in$ B[$\Gamma$,z $\in$ $\sum\limits_{x \epsilon B}$C] }
\end{prooftree}
\end{adjustwidth}
\noindent
\normalsize
...
\noindent
Per individuare correttamente $\pi_1$(z) e $\pi_2$(z) assumo che $\sum\limits_{x \epsilon B}$C(x) type[$\Gamma$] sia derivabile. Tale ipotesi \`e vincolante per l'ottenimento della famiglia. In questo modo
\begin{enumerate}
...
\LeftLabel{F-$\Sigma$}
\UnaryInfC{$\sum\limits_{x \epsilon B}$C(x)[$\Gamma$]}
\LeftLabel{F-c}\RightLabel{(z $\in$ $\sum\limits_{x \epsilon B}$C(x)) $\notin$ $\Gamma$}
\UnaryInfC{$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x) cont}
\LeftLabel{ind-ty}
\BinaryInfC{B type[$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x)]}
...
\LeftLabel{E-$\Sigma_{dip}$}
\BinaryInfC{El$_\Sigma$(z,(x,y).x) $\in$ B[$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x)]}
\end{prooftree}
\end{adjustwidth}
\noindent
\normalsize
In conclusione
\begin{center}$\pi_1$z ${\overset{\mathit{def}}{=}}$ El$_\Sigma$(z,(x,y).x) $\in$ B[$\Gamma$,z $\in$ $\sum\limits_{x \epsilon B}$C(x)]\end{center}

\item Per la seconda proiezione\\
\noindent
$\pi_2$z ${\overset{\mathit{def}}{=}}$ El$_\Sigma$(z,(x,y).y) $\in$ C($\pi_1(z)$)[$\Gamma$,z $\in$ $\sum\limits_{x \epsilon B}$C(x)]\\
\noindent
Allora usando che M(z) $\equiv$ C($\pi_1$z) $\Rightarrow$ M($<$x,y$>$) $\equiv$ C($\pi_1<$x,y$>$)\\
\noindent
\textit{Assumo che $\sum\limits_{x \epsilon B}$C(x) type[$\Gamma$] sia derivabile}
\small
\begin{adjustwidth}{-14em}{}
\begin{prooftree}
\AxiomC{($\ast$)}
\UnaryInfC{$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x) cont}
\LeftLabel{var}
\UnaryInfC{z $\in$ B[$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x)]}
\LeftLabel{PJ$_1$-x}
\UnaryInfC{$\pi_1$z $\in$ B[$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x)]}
...
\LeftLabel{ind-ty}
\BinaryInfC{B type[$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x)]}
\LeftLabel{F-c}\RightLabel{(x $\in$ B) $\notin$ ($\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x))}
\UnaryInfC{$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x), x $\in$ B cont}
\LeftLabel{ind-ty}
\BinaryInfC{C(x) type[$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x), x $\in$ B]}
\LeftLabel{sub-typ}
\BinaryInfC{C($\pi_1$z) type[$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x)]}
...
\UnaryInfC{C(x) $=$ C($\pi_1<$x,y$>$)[$\Gamma$,x $\in$ B,y $\in$ C(x)]}
\LeftLabel{conv}
\BinaryInfC{y $\in$ C($\pi_1<$x,y$>$)[$\Gamma$,x $\in$ B,y $\in$ C(x)]}
\LeftLabel{E-$\Sigma_{dip}$}
\BinaryInfC{El$_\Sigma$(z,(x,y).y) $\in$ C($\pi_1$z)[$\Gamma$, z $\in$ $\sum\limits_{x \epsilon B}$C(x)]}
\end{prooftree}
\end{adjustwidth}
\noindent
\normalsize
