...
\AxiomC{[$\Gamma$] cont}
\UnaryInfC{t{\scriptsize $^I$} $\in$ \textit{T}[$\Gamma$]}
\DisplayProof
...
\AxiomC{c  $\in$ M(t{\scriptsize $^I$})[$\Gamma$]}
...
\AxiomC{M(z) type[$\Gamma$, z $\in$ \textit{T}]}
\AxiomC{c $\in$ M(t{\scriptsize $^I$})[$\Gamma$]}
\BinaryInfC{El$_{\textit{T}}$(t{\scriptsize $^I$}, c) = c $\in$ M(t{\scriptsize $^I$})[$\Gamma$]}
\DisplayProof
...
Tali regole sono del tipo k e rispettano la forma\\
\AxiomC{t = s $\in$ N$_1$[$\Gamma$]}
\AxiomC{M(z) type[$\Gamma$, z $\in$ N$_1$]}
\AxiomC{c = d $\in$ M(t{\scriptsize $^I$})[$\Gamma$]}
\TrinaryInfC{El$_{\textit{T}}$(t{\scriptsize $^I$}, c) = c $\in$ M(t{\scriptsize $^I$})[$\Gamma$]}
\DisplayProof
\end{enumerate}
\noindent
...
t$_1$ $\rightarrow$ $_1$ t$_2$ $\equiv$ t$_1$, ovvero t$_1$ si riduce in un passo di computazione a t$_2$. Ecco che esiste una relazione che computa t$_1$ con t$_2$.\\
La relazione viene definita all'interno dei termini con l'uso delle seguenti regole:
\\\\
\begin{itemize}
\item $\beta$-red) El$_{N1}$($\ast$, t) $\rightarrow_1$ t
...
\end{itemize}
\noindent
\textit{$\beta$-red} risulta valida per \textit{C-S}, le regole di riduzione red$_{I}$ e red$_{II}$ per \textit{eq-E-S}.\\\\
...
In N$_1$ sono vere le seguenti regole di $\beta$-red, indispensabili per provare quali termini sono uguali definizionalmente: 
\begin{itemize}
\item \textit{El$_{N1}$($\ast$, $\ast$) $\rightarrow$ $_1$ $\ast$}
\item \textit{El$_{N1}$($\ast$, x) $\rightarrow$ $_1$ x}
\end{itemize}
\noindent
Inoltre vale che ogni teoria \text{T} $\rightarrow$ $_1$ non \`e deterministica.
\\\\
...
\end{enumerate}
Dai punti elencanti sopra trova validit\`a la relazione \textbf{a \textit{Red$_{NF}$ }NF(a)} $\Rightarrow$ a = NF(a) $\in$ A[$\Gamma$] \`e derivabile (la forma normale \`e uguale al termine stesso). Dunque esiste P programma tale che P(a) = NF(a), per ogni a termine \textit{untyped} in \textit{T$_1$}.
...
\end{prooftree}
\begin{prooftree}
\AxiomC{t{\scriptsize $^I$} $\in$ N$_1$[$\sum$] }
\AxiomC{M(w) type [$\Gamma$, w $\in$ N$_1$]}
\AxiomC{d{\scriptsize $^I$} $\in$ M($\ast$)[$\sum$] }
\LeftLabel{E-S)}
\TrinaryInfC{El$_{N1}$(t{\scriptsize $^I$}, d{\scriptsize $^I$}) $\in$ M(t{\scriptsize $^I$})[$\Gamma$]}
\end{prooftree}
\textbf{Soluzione}\\\\
...
\begin{prooftree}
\AxiomC{}
\UnaryInfC{t{\scriptsize $^I$} $\in$ N$_1$[$\Gamma$]}
\AxiomC{}
\UnaryInfC{M(w) type [$\Gamma$, w $\in$ N$_1$]}
\AxiomC{}
\UnaryInfC{d $\in$ D($\ast$)[$\Gamma$]}
\LeftLabel{E-N$_{1prog}$}
\BinaryInfC{El$_{N1}$(w, d{\scriptsize $^I$}) $\in$ M(w)[$\Gamma$, w $\in$ N$_1$]}
\LeftLabel{sub-typ}
\BinaryInfC{El$_{N1}$(t{\scriptsize $^I$}, d{\scriptsize $^I$}) $\in$ M(t{\scriptsize $^I$})[$\Gamma$]}
\end{prooftree}
