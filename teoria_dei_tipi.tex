...
Le sanitary checks sono regole strutturali derivabili utili per abbreviare le derivazioni. Queste sono derivabili solo se la teoria dei tipi \`e corretta.\\\\
\noindent
T teoria dei tipi di riferimento.\\
\begin{prooftree}
\AxiomC{[$\Gamma$, $\Delta$] cont}
\UnaryInfC{$\Gamma$ cont}
\end{prooftree}
Se [$\Gamma$, $\Delta$] cont \`e derivabile in T allora anche [$\Gamma$] cont \`e derivabile in T.
\begin{prooftree}
\AxiomC{J$_1$,...,J$_n$}
\UnaryInfC{J}
\end{prooftree}
Se J$_i$ con i = 1,...,n in T sono derivabili allora anche J \`e derivabile in T.
\begin{prooftree}
\AxiomC{A type [$\Gamma$]}
\UnaryInfC{$\Gamma$ cont}
\end{prooftree}
Se A type [$\Gamma$] \`e derivabile in T allora anche $\Gamma$ cont \`e derivabile in T.
\begin{center}
...
\DisplayProof
\end{center}
Se A = B type [$\Gamma$] \`e derivabile in T allora anche A type[$\Gamma$] e B type[$\Gamma$] sono derivabili in T.
\begin{prooftree}
\AxiomC{a $\in$ A type [$\Gamma$]}
\UnaryInfC{A type[$\Gamma$]}
\end{prooftree}
Se a $\in$ A type [$\Gamma$] \`e derivabile in T allora anche A type[$\Gamma$] \`e derivabile in T.
\begin{center}
...
\DisplayProof
\end{center}
Se a = b $\in$ A type [$\Gamma$] \`e derivabile in T allora anche a $\in$ A type[$\Gamma$] e b $\in$ A type[$\Gamma$] sono  derivabile in T.
\section{Esercizi}
