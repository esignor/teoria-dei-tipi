...
\AxiomC{[$\Gamma$] cont}
\LeftLabel{I-S)}
\UnaryInfC{$\ast$ $\in$ $N_1$ type[$\Gamma]$ cont}
\end{prooftree}
...
\AxiomC{c  $\in$ M($\ast$) type[$\Gamma$]}
\LeftLabel{E-S)}
\TrinaryInfC{$El_{N1}$(t, c) $\in$ M($\ast$) type[$\Gamma$]}
\end{prooftree}
\noindent
...
\AxiomC{c $\in$ M($\ast$) type[$\Gamma$]}
\LeftLabel{C-S)}
\BinaryInfC{$El_{N1}$($\ast$, c) = c $\in$ M($\ast$) type[$\Gamma$]}
\end{prooftree}
...
Identificate con il preambolo F-\textit{T} con \textit{T} teoria dei tipi in esame.\\Tali regole sono del tipo k e rispettano la forma 
\AxiomC{[$\Gamma$] cont}
\UnaryInfC{\textit{T} type[$\Gamma$]}
\DisplayProof
\item \textbf{Regole di introduzione}
identificate con il preambolo I-\textit{T} con \textit{T} teoria dei tipi in esame.\\Tali regole consistono nella forma introduttiva degli elementi canonici di \textit{T} e rispettono la forma
\AxiomC{[$\Gamma$] cont}
\UnaryInfC{t $\in$ \textit{T} type[$\Gamma$]}
\DisplayProof
\item \textbf{Regole di eliminazione}
identificate con il preambolo E-\textit{T} con \textit{T} teoria dei tipi in esame.\\Tali regole sono definenti E$_k$, a partire dagli elementi di k a valori in un tipo M(z) type[$\Gamma$, z $\in$k]. L'ipotesi valida \`e che siano dati degli elementi in M(z) sui valori canonici di k.
Tali regole sono del tipo k e rispettano la forma 
\AxiomC{t $\in$ \textit{T} type[$\Gamma$]}
\AxiomC{M(z) type[$\Gamma$, z $\in$ \textit{T}]}
\AxiomC{c  $\in$ M($\ast$) type[$\Gamma$]}
\TrinaryInfC{$El_{\textit{T}}$(t, c) $\in$ M($\ast$) type[$\Gamma$]}
\DisplayProof
\item \textbf{Regole di conversione}
identificate con il preambolo C-\textit{T} con \textit{T} teoria dei tipi in esame.\\, che stabilscono che gli eliminatori in (3) sono stabiliti per ricorsione a partire dalle ipotesi.
Tali regole sono del tipo k e rispettano la forma
\AxiomC{M(z) type[$\Gamma$, z $\in$ \textit{T}]}
\AxiomC{c $\in$ M(t) type[$\Gamma$]}
\LeftLabel{C-S)}
\BinaryInfC{El$_{\textit{T}}$(t, c) = c $\in$ M(t) type[$\Gamma$]}
\DisplayProof
...
\LeftLabel{var}
\UnaryInfC{w $\in$ N$_1$ type[$\Gamma$, w $\in$ N$_1$]}
\AxiomC{}
\UnaryInfC{M(z) type[$\Gamma$, z $\in$ N$_1$, w $\in$ N$_1$]}
\AxiomC{}
\UnaryInfC{d $\in$ M($\ast$) type[$\Gamma$, w $\in$ N$_1$]}
\LeftLabel{E-S}
...
\AxiomC{}
\UnaryInfC{z $\in$ $N_1$ type[$\Gamma$, z $\in$ $N_1$, w $\in$ N$_1$]}
\LeftLabel{sub-typ}
...
\begin{prooftree}
\AxiomC{}
\UnaryInfC{t' $\in$ N$_1$ type[$\Gamma$]}
\AxiomC{}
\UnaryInfC{M(w) type [$\Gamma$, w $\in$ N$_1$]}
\AxiomC{}
\UnaryInfC{d $\in$ D($\ast$) type[$\Gamma$]}
\LeftLabel{E-N$_{1prog}$}
\BinaryInfC{El$_{N1}$(w, d') $\in$ M(w) type[$\Gamma$, w $\in$ N$_1$]}
\LeftLabel{sub-typ}
\BinaryInfC{El$_{N1}$(t', d') $\in$ M(t') type[$\Gamma$]}
\end{prooftree}
