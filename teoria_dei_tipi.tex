...
\AxiomC{M(z) type[$\Gamma$, z $\in$ \textit{T}]}
\AxiomC{c $\in$ M(t')[$\Gamma$]}
\BinaryInfC{El$_{\textit{T}}$(t', c) = c $\in$ M(t')[$\Gamma$]}
\DisplayProof
...
\AxiomC{c = d $\in$ M(t')[$\Gamma$]}
\TrinaryInfC{El$_{\textit{T}}$(t', c) = c $\in$ M(t')[$\Gamma$]}
\DisplayProof
\end{enumerate}
\noindent
...
\UnaryInfC{z $\in$ $N_1$[$\Gamma$, z $\in$ $N_1$, w $\in$ N$_1$]}
\end{prooftree}
Assumo che le premesse di \textit{E-N$_1prog$} (\textit{M(w) type [$\Gamma$,w $\in$ N$_1$]} e \textit{d $\in$ M($\ast$)[$\Gamma$]}) siano valide, p\`erci\`o \`e valido, dalla prova sopra, anche il giudizio di conclusione \textit{El$_{N1}$(w, d) $\in$ M(w)[$\Gamma$, w $\in$ N$_1$]}, di conseguenza derivabile in \textit{T$_1$}.
...
\end{prooftree}
\begin{prooftree}
\AxiomC{t` $\in$ N$_1$[$\sum$] }
\AxiomC{M(w) type [$\Gamma$, w $\in$ N$_1$]}
\AxiomC{d` $\in$ M($\ast$)[$\sum$] }
\LeftLabel{E-S)}
\TrinaryInfC{El$_{N1}$(t`, d`) $\in$ M(t`)[$\Gamma$]}
\end{prooftree}
\textbf{Soluzione}\\\\
...
\BinaryInfC{El$_{N1}$(w, d') $\in$ M(w)[$\Gamma$, w $\in$ N$_1$]}
\LeftLabel{sub-typ}
\BinaryInfC{El$_{N1}$(t', d') $\in$ M(t')[$\Gamma$]}
\end{prooftree}
